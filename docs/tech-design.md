...
- `initializeDigesters()` registers all digest pipelines (URL crawl, summary, tagging, slugging, OCR) and ensures the registry knows which MIME types or folders apply.
- `DigestCoordinator` inspects a file, determines eligible digesters, creates pending digest rows, and either runs synchronous digesters or enqueues asynchronous ones via the task queue.
- `sqlar_name` is assigned for binary artifacts so screenshots, OCR PDFs, or AI attachments never clutter the user library.
