...
- **Coordinator flow:** `DigestCoordinator.processFile()` loads file metadata + existing digests, ensures placeholders exist (status=`todo`), loops over registered digesters, skips already terminal outputs, marks pending ones as `in-progress`, then runs the digester. Results persist immediately so partial progress survives crashes.
...
- **Backfill + evolution:** `initializeDigesters()` registers the current set, `syncNewDigesters()` inserts `todo` rows for every existing file that already had digests (not just inbox items), and the lazy-insert model ensures only files that need work get rows. Adding a new digester is as simple as dropping an implementation under `src/lib/digest/digesters`, registering it, and restarting; the supervisor immediately starts draining the new backlog.
- **TODOs:**
