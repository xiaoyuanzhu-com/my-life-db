...
- **Batching:** Task handlers fetch up to N file paths via `findFilesNeedingDigestion()` (ordering by oldest `updated_at`) and process each file through the full digester chain before moving on. `digest_file` and `digest_batch` tasks both reuse the same coordinator logic.
...
- **Backfill + evolution:** `initializeDigesters()` registers the current set, `syncNewDigestTypes()` inserts pending digests when new processors ship, and the lazy-insert model ensures only files that need work get rows. Adding a new digester is as simple as placing it in `src/lib/digest/digesters`, registering it, and restarting; the batch handler will pick up the new pending rows automatically.
...
- **Home / OmniInput (`src/app/page.tsx`, `src/components/omni-input.tsx`):** Floating composer that persists text in `sessionStorage`, detects input type, performs adaptive debounce search, accepts drag-and-drop files, and surfaces instant search results beneath the textarea via `SearchResults`.
- **Inbox (`src/app/inbox/page.tsx`):** Groups inbox files by local date, renders cards with snippets via `FileCard`, and keeps sticky day headers so long timelines remain scannable even while digest statuses update.
...
- **Search overlay (`src/components/search-results.tsx`):** Lightweight list that mirrors the search design doc, showing highlighted snippets and linking directly to file inspectors or library tabs.
...
- **Shared chrome:** `Header`, `BottomNav`, `Footer`, and `ThemeToggle` live under `src/components/`, keep navigation consistent, and respect design tokens defined in `globals.css`.
