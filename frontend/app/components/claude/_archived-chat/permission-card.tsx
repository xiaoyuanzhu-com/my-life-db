...
export function PermissionCard({ request, onDecision, isFirst = true }: PermissionCardProps) {
  ...
  return (
    ...
    >
      ...
      {isExitPlanMode ? (
        <div
          className="rounded-lg border border-border p-4 mb-3 max-h-48 md:max-h-96 overflow-y-auto overflow-x-hidden break-words prose-claude"
          style={{ backgroundColor: 'var(--claude-bg-code-block)' }}
          dangerouslySetInnerHTML={{ __html: planHtml }}
        />
      ) : (
        <div
          className="rounded-lg border border-border p-2 font-mono text-[12px] text-foreground overflow-x-hidden mb-3 max-h-24 md:max-h-32 overflow-y-auto"
          style={{ backgroundColor: 'var(--claude-bg-code-block)' }}
        >
