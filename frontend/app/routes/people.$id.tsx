...
export default function PeopleDetailPage() {
  ...
  const handleSaveName = async () => {
    ...
    const matchingPerson = existingPeople.find((p) => p.displayName.toLowerCase() === editName.trim().toLowerCase());
  ...
  return (
    <div className="min-h-screen bg-background">
      <div className="px-[20%] py-8">
        {/* Back link */}
        <Link to="/people" className="inline-flex items-center text-muted-foreground hover:text-foreground mb-6">
          <ArrowLeft className="mr-2 h-4 w-4" />
          Back to People
        </Link>
            ...
            {isEditing ? (
              <div className="relative">
                ...
                {showSuggestions &&
                  editName.trim() &&
                  (() => {
                    const filtered = existingPeople.filter((p) => fuzzyMatch(editName.trim(), p.displayName)).slice(0, 10);
            ...
            <AlertDialog open={!!mergeTargetId} onOpenChange={(open) => !open && setMergeTargetId(null)}>
              <AlertDialogContent>
                <AlertDialogHeader>
                  ...
                  <AlertDialogDescription>
                    &quot;{mergeTargetName}&quot; already exists. Do you want to merge this person into them? All voice
                    clips and faces will be moved to {mergeTargetName}.
                  </AlertDialogDescription>
                </AlertDialogHeader>
                <AlertDialogFooter>
              ...
              <AlertDialog>
                ...
                <AlertDialogContent>
                  <AlertDialogHeader>
                    ...
                    <AlertDialogDescription>
                      This will delete {people.displayName || "this entry"} and all their clusters. The underlying
                      embeddings will be kept but unassigned.
                    </AlertDialogDescription>
                  </AlertDialogHeader>
                  <AlertDialogFooter>
              ...
              <div className="grid grid-cols-4 md:grid-cols-6 lg:grid-cols-8 gap-2">
                {people.embeddings.face.map((emb) => (
                  <div key={emb.id} className="relative group">
                    ...
                    <p className="text-xs text-muted-foreground truncate mt-1">{emb.sourcePath.split("/").pop()}</p>
                  </div>
                ))}
              </div>
            </CardContent>
        ...
        {people.embeddings.voice.length === 0 && people.embeddings.face.length === 0 && (
          <Card>
            <CardContent className="text-center py-12">
              <p className="text-muted-foreground">No voice clips or faces linked to this people entry yet.</p>
            </CardContent>
          </Card>
        )}
      </div>
