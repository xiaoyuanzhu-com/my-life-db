...
This is the Go server for MyLifeDB, migrated from Node.js/Express.
...
cd go-server
make build-server
# Or: go build -o ../bin/server ./cmd/server
```
...
npm run dev:go
# Or: cd go-server && go run ./cmd/server
```
...
1. The existing SQLite database is fully compatible
2. Environment variables use the same names
3. All API responses maintain the same format
4. The React frontend works unchanged
