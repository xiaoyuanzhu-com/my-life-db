...
func (h *Handlers) AgentSessionWebSocket(c *gin.Context) {
	...
	for {
		...
		case "session.prompt":
			...
			// Send prompt and stream frames. See agent_prompt_turn.go for the
			// full lifecycle; this path's only specific contribution is the
			// "ws" source label and SetPromptInFlight above.
			go h.agentMgr.RunPromptTurn(promptCtx, pCancel, done, acpSession, sessionState, sessionID, promptText, "ws")
