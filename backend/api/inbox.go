...
// GetInboxItemDebug handles GET /api/inbox/:id/debug
// Returns raw DB values including null fields for debugging
func (h *Handlers) GetInboxItemDebug(c *gin.Context) {
	id := c.Param("id")
	path := "inbox/" + id
	...
	// Return raw values without omitempty - explicitly show null fields
	hashValue := "<nil>"
	if file.Hash != nil {
		hashValue = *file.Hash
		if hashValue == "" {
			hashValue = "<empty string>"
		}
	}

	textPreviewValue := "<nil>"
	if file.TextPreview != nil {
		if *file.TextPreview == "" {
			textPreviewValue = "<empty string>"
		} else if len(*file.TextPreview) > 100 {
			textPreviewValue = (*file.TextPreview)[:100] + "..."
		} else {
			textPreviewValue = *file.TextPreview
		}
	}

	// Check if file is considered a text file
	isTextFile := h.server.FS().IsTextFile(path)
