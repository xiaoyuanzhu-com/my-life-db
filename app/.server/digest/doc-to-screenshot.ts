/**
 * Document to Screenshot Conversion
 * Uses HAID service to convert documents to screenshot images
 */
...
const log = getLogger({ module: 'DocToScreenshot' });
const DEFAULT_BASE_URL = 'http://172.16.2.11:3003';
const DEFAULT_LIB = 'screenitshot/screenitshot';

export interface DocToScreenshotOptions {
  filePath: string;
  filename: string;
  lib?: string;
}

export interface DocToScreenshotResponse {
  requestId: string;
...
/**
 * Convert a document file to screenshot using HAID service
 */
export async function convertDocToScreenshot(
  options: DocToScreenshotOptions
): Promise<DocToScreenshotResponse> {
  if (!options.filePath) {
    throw new Error('convertDocToScreenshot requires a filePath');
  }

  if (!options.filename) {
    throw new Error('convertDocToScreenshot requires a filename');
  }
  ...
  log.debug(
    ...
    'converting document to screenshot'
  );

  const config = await resolveHaidConfig();
  const endpoint = `${config.baseUrl}/api/doc-to-screenshot`;
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `HAID doc-to-screenshot error (${response.status}): ${errorText || response.statusText}`
    );
  }
  ...
  if (!data.screenshot) {
    throw new Error('HAID doc-to-screenshot response did not include screenshot');
  }
  ...
  log.info(
    ...
    'document converted to screenshot'
  );
